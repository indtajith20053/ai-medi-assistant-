...
================================================
Pure Python conversion of Medical_AI_Assistant_v2.ipynb

A LangGraph-based multi-agent medical information assistant that routes user
questions to specialist agents (symptom assessment, medical knowledge RAG,
medication info, or general chat), backed by a WHO-reference PDF knowledge
base indexed in ChromaDB with MMR retrieval, and served through a Gradio UI.

Usage:
    python medical_ai_assistant.py

Requires a `.env` file (in the same folder, or set OPENROUTER_API_KEY as an
environment variable) containing:
    OPENROUTER_API_KEY=sk-or-v1-xxxxxxxxxxxxxxxxxxxxxxxx

Also requires a WHO reference PDF at:
    knowledge-base/9789240110496-eng (1).pdf
(relative to this script's working directory) the first time the vector
store is built.
"""
...
try:
    ...
    # Safe to skip on CPU-only Spaces or local runs where it isn't installed.
...
# ---------------------------------------------------------------------------
MODEL_CONFIG = {
    "supervisor": "tencent/hy3:free",
    "symptom": "tencent/hy3:free",
    "medication": "tencent/hy3:free",
    "rag": "nvidia/nemotron-3-ultra-550b-a55b:free",
}
FALLBACK_MODEL = "tencent/hy3:free"
...
def build_or_load_vectordb() -> Chroma:
    ...
    # If the collection is already populated, reuse it as-is.
    try:
...
def supervisor_route(user_message: str, chat_history_text: str = "") -> str:
    ...
    if route not in VALID_ROUTES:
        # LLM gave something unexpected — fall back to the keyword pre-check, then "general"
